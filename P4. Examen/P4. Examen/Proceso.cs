...
namespace P4.Examen
{
    class Proceso
    {
        ...
        public Proceso(double[] Array) //Recibimos como párametro el Vector ya inicializado de la Clase.
        {
            ...
            QuickSort(Array, 0, Array.Length - 1); //Llamamos al método Play que ejecuta el algoritmo necesario para realizar el QuickSort.
            Desplegar();
        }

        private void QuickSort(double[] array, int primero, int ultimo) //Recibe como párametros el Vector, el primer índice y el último índice del vector.
        {
            ...
            if (primero < y)
            {
                QuickSort(array, primero, y); //De manera recursiva, si el primero es menor al segundo, se mandan primero los párametros de el primero y el segundo elemento.
            }
            if (x < ultimo)
            {
                QuickSort(array, x, ultimo); //Y cuando el primero es menor que el último, se mandan despues los párametros de el primero y el último.
            }
        }
